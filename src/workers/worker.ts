...
export type EnumOptionMap = {
    ...
    brand: EnumOption[]
    category: EnumOption[]
    ewlinkSupported: EnumOption[]
    ewlinkCapabilities: EnumOption[]
...
function passEnums(row: FlatRow, enums?: EnumFilters): boolean {
    ...
    if (enums.ewlinkSupported?.length) {
        if (!enums.ewlinkSupported.includes(row.ewlinkSupported)) return false
    }
    if (!includesAny(enums.ewlinkCapabilities, row.ewlinkCapabilities)) return false
...
const api = {
    ...
    async distinct(): Promise<EnumOptionMap> {
        ...
        return {
            ...
            ewlinkSupported: collect(row => row.ewlinkSupported, true),
            ewlinkCapabilities: collectArray(row => row.ewlinkCapabilities),
